...
module.exports = new Event("messageCreate", async (client, message) => {
	...
	if(message.mentions.users.first()) {
		if (message.mentions.users.first().id === "ur bot id goes here") return message.reply(`:wave: **| Prefix hiện tại của server là \`${p}\`!**`)
	} 
	...
	if (!command) return message.channel.send({ content: `**:x: | \`${cmdName}\` không phải là lệnh hợp lệ!**` });
